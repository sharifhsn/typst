...
pub(crate) struct GlobalContext<'a> {
    ...
    /// keyed by the SVG's content hash. This both avoids re-converting a repeated
    /// SVG (e.g. a per-page logo) and lets krilla emit it once and reference it
    /// via `/Do` on each use, rather than inlining its drawing operations every
    /// time.
    pub(crate) svg_graphics: FxHashMap<u128, krilla::graphic::Graphic>,
