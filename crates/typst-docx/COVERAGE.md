...
## 54. Explicit paragraph leading uses Typst's text-frame metrics
...
An initial cross-run comparison appeared to disprove the policy: Xenolay's
161-page reference had rendered as 163 pages in one run and 129 in the
experiment. The DOCX packages from those runs are in fact byte-identical
(`5734cb7a...acae22`). The 163-page result used LibreOffice 26.2.4.2, while the
129-page result used LibreOfficeDev 26.8 alpha. Converting that same package
again with 26.2.4.2 reproduces 163 pages, so the page jump was consumer-version
behavior and could not evidence an exporter regression.

Matched-consumer conversion with LibreOffice 26.2.4.2 instead confirms the
intended effect: nominal-leading Mathnote remains 187 pages and the text-frame
package is 171, while Xenolay remains 163 with identical bytes. The mapper now
uses the resolved font top/bottom-edge frame plus authored leading, falling back
to nominal size for bounds-based edges or unavailable fonts. The focused
experiment remains under `target/docx-public-corpus-focus-leading-ee1ccd4/`;
matched current evidence is under
`target/docx-public-corpus-focus-centered-grid-stable/`. The retained combined
policy is under `target/docx-public-corpus-focus-leading-grid-stable/`: all six
high-risk packages, LibreOffice 26.2 renders, and review round trips pass.
Mathnote improves from 187 to 171 pages (`0.976302`), Xenolay stays 163,
Tura stays at its improved 293, and gb-ctr stays 202. The previously hanging
table/footnote thesis also improves from 212 to 195 pages, while a missing-font
C++ guide remains exactly one page. These comparisons use the same consumer and
tool identities throughout.
...
resume continues to preserve the artifact's original identity. Resume also now
rejects changed `soffice`, `pdftotext`, or `pdftoppm` identities so one authority
directory cannot silently mix consumer versions.

## 55. Centered layout-grid insets do not expand measured rows twice

Tura's 420 Word tables and 6,389 rows initially looked like a semantic-table
pagination failure. Source and package inspection instead traced nearly all of
them to the editable two-column layout grids generated by roughly 280 Codly code
blocks. In the dominant 5,561 short rows, each 6.75pt measured content minimum
was accompanied by equal 4.2pt top and bottom cell margins and centered vertical
alignment. Typst's 15.15pt physical row already included that inset, but Word
added `w:tcMar` outside the emitted minimum and repeatedly expanded the flow.

Counterfactuals established the boundary before changing the mapper. Making all
short rows exact reached 225 pages against the 265-page reference but could clip
editable text. Removing all vertical margins reached 281 pages but discarded the
authored padding. Exact paragraph line boxes retained the margins but reached 298
pages and carried the same clipping risk. None was retained.

The mapper now preserves whether a resolved `CellGrid` came from a semantic
`TableElem` or a layout `GridElem`; it does not infer provenance from geometry or
package names. For a measured layout-grid row only, when every independent cell
has symmetric vertical inset and every nonzero inset already uses centered
alignment, the full measured physical row becomes the editable `atLeast` minimum
and the redundant top/bottom `w:tcMar` values become zero. Semantic tables,
asymmetric or non-centered grid cells, spans, unmeasured grids, and explicit
gutter rows retain their prior behavior.

The exact Tura export opens and converts in LibreOffice at 293 pages instead of
351, removing 58 of 86 excess pages while retaining 41,866 extracted DOCX words,
`0.991067` semantic text coverage, score `0.975289`, and a successful 6,880-region
review round trip. Delegated visual QA compared matched code-heavy passages with
the 265-page Typst PDF and found no new clipping, glyph overlap, truncated lines,
or collapsed row separation. A paired structural regression proves that centered
layout grids use their full measured row with zero redundant vertical margins
