    ...
    fn new_impl(
        ...
        let inner = match format {
            RasterFormat::Exchange(exchange) => {
                ...
                if exchange == ExchangeFormat::Jpg {
                    // JPEGs are embedded losslessly into PDF and SVG output, so
                    // their pixels are only needed when rasterizing (e.g. PNG
                    // export). Read just the dimensions and ICC profile from the
                    // header now and defer the expensive pixel decode to the first
                    // `dynamic()` access.
                    ...
                    // Other formats are always decoded for output, so decode
