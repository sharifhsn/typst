...
| **Table styles (`w:tblStyle`)** | ✗ | ✅ | Export never names a style (it writes direct formatting). Import resolves the named style through its `basedOn` chain into the table's borders and cell-padding defaults, plus the cell shading and vertical alignment it gives every cell — each still overridden by anything the table or cell states directly. This is not a nicety: Word's built-in **`TableGrid`** carries a table's whole appearance in the style and nothing on the table, and it accounts for **5273 of the ~5400 table-style references** in the wide corpus, so those tables were arriving borderless and wearing Typst's default grid. `w:tblStylePr` conditional formatting (banded rows, first row/column) is reported, not applied — 58 of the 305 corpus documents that name a table style use it. |
| Table-level cell margins (`w:tblCellMar`) | ✅ | ✅ | The default padding every cell inherits and its own `w:tcMar` overrides. Read from both the table's own `w:tblPr` and its style's. |
