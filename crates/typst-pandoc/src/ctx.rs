...
impl<'a, 'e> PandocCtx<'a, 'e> {
    ...
    pub fn anchor_id(&self, loc: typst_library::introspection::Location) -> EcoString {
        ...
        let introspector = self.engine.introspector.access(
            "reading an element's label to derive its anchor id is a pure query",
        );
...
mod tests {
    ...
    fn reserved_ref_form_is_guarded() {
        assert_eq!(
            sanitize_label_id("ref-00000000deadbeef"),
            "L-ref-00000000deadbeef"
        );
