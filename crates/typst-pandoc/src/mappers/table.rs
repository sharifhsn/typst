...
fn cellgrid(
    ...
    let is_header_row = |y: usize| -> bool {
        grid.headers
            .iter()
            .any(|hd| header_range(grid, &hd.range).contains(&y))
    };
...
fn build_cell(
    ...
    Ok(Cell(ast::empty_attr(), align, rowspan as i32, colspan as i32, blocks))
}
...
fn header_range(
    ...
    if grid.has_gutter { range.start / 2..range.end.div_ceil(2) } else { range.clone() }
}
...
fn footer_range(
    ...
    if grid.has_gutter { range.start / 2..range.end.div_ceil(2) } else { range }
}
...
mod tests {
    ...
    fn table_six_tuple_with_rowspan_omission() {
        ...
        let body_rows = vec![
            ...
            Row(ast::empty_attr(), vec![plain_cell("b", 1, 1, Alignment::AlignDefault)]),
        ];
        let bodies = vec![TableBody(ast::empty_attr(), 0, Vec::new(), body_rows)];
